...
class ControllerModuleMenu extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($this->request->post['act_mode']){
				...
				$this->redirect($this->url->link('module/menu', 'token=' . $this->session->data['token'], 'SSL'));
			}
		}
		...
		$this->data['text_content_menu'] = $this->language->get('text_content_menu');
		...
		$this->data['cancel'] = $this->url->link('extension/menu', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->data['menus']=$this->model_menu_menus->getMenus();
	...
	private function validate() {
	 ...
	 public function install(){
	 	$this->load->model('module/menu');
	 	if (!$this->user->hasPermission('modify', 'module/menu')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		$this->model_module_menu->install();	
	} 
 	...
 	public function uninstall(){
		...
		$this->model_module_menu->uninstall();	
	} 
}
?>